...
const register = async(req, res) => {
    try {
        ...
        // MÓDOSÍTVA: Átirányítás helyett JSON üzenetet küldünk, amit a frontend kezel le
        return res.status(200).json({ status: "success", redirect: '/website0.html' });
    } catch (err) {
        console.error(err);
        return res.status(500).json({ error: "Szerverhiba történt a regisztráció során." });
    }
};
...
const userDelete = async (req, res) => {
    try {
        ...
        if (res.clearCookie) {
            ...
            res.cookie('cartItems', '', { httpOnly: true, expires: new Date(0) });
        }
...
const login = async (req, res) => {
    try {
        ...
        // MÓDOSÍTVA: Átirányítás helyett itt is JSON választ adunk vissza a frontendnek
        return res.status(200).json({ status: "success", redirect: '/website0.html' });
    } catch (err) {
        console.error(err);
        return res.status(500).json({ error: "Szerverhiba történt a bejelentkezés során." });
    }
};
...
module.exports = { login, register, logout, userDelete, deleteAccount: userDelete, getCurrentUser };
